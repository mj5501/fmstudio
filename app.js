...
const defaults={bg:'#f6f7f8',text:'#272b31',accent:'#4b5563',activeText:'#ffffff',button:'#ffffff',border:'#d5d9df',radius:16,columns:2,buttonMaxWidth:280,fontSize:14,padding:20,maxWidth:760,mobileColumns:2,mobileFontSize:14,mobilePadding:14,theme:'clean',textAlign:'left',buttonAlign:'left',buttonTextAlign:'center'};
const themePresets={
...
function validate(p){
 ...
 for(const k of ['mobileColumns','mobileFontSize','mobilePadding','maxWidth','buttonMaxWidth'])if(p.design[k]===undefined)p.design[k]=defaults[k];
 ...
 for(const [k,min,max] of [['radius',0,36],['columns',1,8],['mobileColumns',1,8],['mobileFontSize',11,24],['mobilePadding',8,40],['fontSize',11,24],['padding',8,40],['maxWidth',280,1600],['buttonMaxWidth',80,800]])if(!Number.isInteger(p.design[k])||p.design[k]<min||p.design[k]>max)throw Error('디자인 수치 범위를 확인해 주세요.');
