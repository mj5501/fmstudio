...
const defaults={bg:'#f6f7f8',text:'#272b31',accent:'#4b5563',activeText:'#ffffff',button:'#ffffff',border:'#d5d9df',radius:16,columns:2,relationButtonMaxWidth:280,openingButtonMaxWidth:280,relationButtonHeight:48,openingButtonHeight:48,fontSize:14,padding:20,maxWidth:760,mobileColumns:2,mobileFontSize:14,mobilePadding:14,theme:'clean',textAlign:'left',buttonAlign:'left',buttonTextAlign:'center'};
const themePresets={
...
function validate(p){
 ...
 if(p.useCollapse===undefined)p.useCollapse=true;
 if(typeof p.languages!=='boolean'||typeof p.useRelations!=='boolean'||typeof p.useCollapse!=='boolean'||typeof p.resetButton!=='boolean')throw Error('버튼 설정이 올바르지 않아요.');
 ...
 for(const k of ['bg','text','accent','activeText','button','border'])if(!/^#[0-9a-f]{6}$/i.test(p.design[k]))throw Error('색상은 6자리 HEX 값이어야 해요.');
 for(const [k,min,max] of [['radius',0,36],['columns',1,8],['mobileColumns',1,8],['mobileFontSize',11,24],['mobilePadding',8,40],['fontSize',11,24],['padding',8,40],['maxWidth',280,1600],['relationButtonMaxWidth',80,800],['openingButtonMaxWidth',80,800],['relationButtonHeight',32,300],['openingButtonHeight',32,300]])if(!Number.isInteger(p.design[k])||p.design[k]<min||p.design[k]>max)throw Error('디자인 수치 범위를 확인해 주세요.');
...
function renderEditor(){
 ...
 const item=r?(o?field('버튼에 표시할 제목','o.title',o.title)+fold('opening-button-image','퍼메 버튼 이미지',buttonImageCard('','o.image',o.image),false)+bodyFields+'<p class="hint">본문의 {{user}}, {{char}} 같은 CBS와 마크다운은 내보낼 때 그대로 보존돼요.</p>':field('관계 이름','r.name',r.name)+fold('relation-button-image','관계 버튼 이미지',buttonImageCard('','r.image',r.image),false)+field('관계 설명','r.description',r.description,'textarea'))+`<div class="actions"><button data-item="up">↑ 위로</button><button data-item="down">↓ 아래로</button><button data-item="duplicate">복제</button><button data-item="delete" class="danger">삭제</button></div>`:'<p class="empty">왼쪽에서 관계를 추가하세요.</p>';
 ...
 const colors=`<div class="grid2">${[['bg','배경색'],['text','글자색'],['accent','선택 색상'],['activeText','선택 글자색'],['button','버튼 배경'],['border','테두리']].map(([k,l])=>field(l,'design.'+k,project.design[k],'color')).join('')}</div>`;
 const sizes=`<div class="grid2">${[['maxWidth','선택기 최대 가로 너비',280,1600],['relationButtonMaxWidth','관계 버튼 최대 너비',80,800],['relationButtonHeight','관계 버튼 최소 높이',32,300],['openingButtonMaxWidth','퍼메 버튼 최대 너비',80,800],['openingButtonHeight','퍼메 버튼 최소 높이',32,300],['radius','둥글기',0,36],['columns','PC 버튼 열 수',1,8],['mobileColumns','모바일 버튼 열 수',1,8],['mobileFontSize','모바일 글자 크기',11,24],['mobilePadding','모바일 안쪽 여백',8,40],['fontSize','글자 크기',11,24],['padding','안쪽 여백',8,40]].map(([k,l,min,max])=>field(l,'design.'+k,project.design[k],'number',`min="${min}" max="${max}"`)).join('')}</div><p class="hint">관계와 퍼메 버튼의 너비·높이를 따로 조절할 수 있어요. 높이는 이미지나 긴 글자가 잘리지 않도록 최소 높이로 적용돼요.</p>`;
 $('#editor').innerHTML=`<div class="actions compact-actions"><button id="save-design">내 기본 디자인 저장</button><button id="apply-design">내 기본 디자인 적용</button></div>`+fold('design-theme','기본 테마',themes,false)+fold('design-alignment','정렬',alignment,false)+fold('design-colors','색상',colors,false)+fold('design-size','크기 · PC/모바일 배치',sizes,true);
 }else if(tab==='images'){
...
function generate(p){
 ...
 const button=(id,title,stateKey,stateValue,image)=>{const cls=stateKey?(stateKey==='language'?(stateValue==='en'?whenNot(key('language'),'ko','fm-active'):when(key('language'),'ko','fm-active')):when(key(stateKey),stateValue,'fm-active')):'',kind=stateKey==='relation'?'fm-relation':stateKey==='opening'?'fm-opening':'',bg=image?.backgroundAsset?` style="background-image:linear-gradient(rgba(0,0,0,${image.backgroundShade/100}),rgba(0,0,0,${image.backgroundShade/100})),url('{{raw::${image.backgroundAsset}}}');background-position:${image.backgroundPosition}"`:'';return `<button class="${kind} ${cls}" risu-trigger="${fn(id)}"${bg}>${assetImg(image?.asset,'fm-thumb')}${labelEsc(title)}</button>`;};
 ...
 if(p.useRelations){inner+=`<div class="fm-grid fm-relation-grid">${p.relations.map(r=>button('rel_'+r.id,r.name,'relation',r.id,r.image)).join('')}</div>`;for(const r of p.relations)inner+=when(key('relation'),r.id,`<div class="fm-note">${labelEsc(r.description)}</div><div class="fm-grid fm-opening-grid">${r.openings.map(o=>button('open_'+o.id,o.title,'opening',o.id,o.image)).join('')}</div>`);}else inner+=`<div class="fm-grid fm-opening-grid">${p.relations.flatMap(r=>r.openings.map(o=>button('open_'+o.id,o.title,'opening',o.id,o.image))).join('')}</div>`;
...
function render(){syncPreviewMode();tree();renderQuickSettings();renderEditor();drawPreview();}
...
document.addEventListener('input',e=>{
 ...
 persist();tree();if(path==='languages')renderEditor();if(path==='useCollapse')renderQuickSettings();try{validate(project);drawPreview();}catch{}
});
